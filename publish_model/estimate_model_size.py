...
def main():
    ...
    print(f"{'BF16 (bfloat16)':<15} {format_bytes(bf16_bytes):<20} Brain float (similar to FP16)")
    
    # Int8 quantization - 1 byte per parameter
    int8_bytes = total_params * 1
    print(f"{'Int8 (quantized)':<15} {format_bytes(int8_bytes):<20} Quantized (lower quality)")
    
    # Memory overhead (optimizer states, activations, etc.)
    print(f"\nAdditional Memory Considerations:")
    print(f"  - Optimizer states: ~2-3x model size (for training)")
    print(f"  - Activations: Varies with batch size and sequence length")
    print(f"  - Gradient memory: ~1x model size (for training)")
    ...
    print(f"\nRecommendations:")
    print(f"  - GPU with >= 40GB VRAM (FP16, batch_size=16)")
    print(f"  - GPU with >= 80GB VRAM (FP16, larger batches)")
    print(f"  - CPU inference possible but much slower")
    print(f"  - Consider quantization for edge devices")
