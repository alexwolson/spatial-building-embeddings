...
Install or refresh the environment with `uv sync` (Python 3.11). All workflows use a unified `config.toml` at the root with a `[global]` section for shared settings and workflow-specific sections (`[process_tar]`, `[merge_and_split]`, `[generate_embeddings]`, `[triplet_training]`, `[difficulty_metadata]`). Copy `config.example.toml` to `config.toml` and customize. Run preprocessing with `uv run python preprocess_raw_data/process_tar.py --config config.toml` followed by `merge_and_split.py` to materialize splits. Generate embeddings for a shard via `uv run python generate_embeddings/generate_embeddings.py --config config.toml` on a GPU-enabled host. Train the projection head with `uv run python train_specialized_embeddings/train.py --config config.toml`. Cluster wrappers reference the root `config.toml` via `slurm/submit_*.sh`.
