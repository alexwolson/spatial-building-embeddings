#!/bin/bash
# Job submission script for computing fingerprints on Nibi cluster.
#
# This script discovers intermediate parquet files, sets up Python environment if needed,
# and submits a SLURM job array to compute fingerprints in parallel.
#
# Usage:
#   ./submit_fingerprint_jobs.sh --account <ACCOUNT>
#
# Options:
...
OUTPUT_DIR=$(read_toml_value "${CONFIG_FILE}" "paths" "fingerprints_dir")

if [ -z "${INTERMEDIATES_DIR}" ]; then error_exit "intermediates_dir not found in [paths] section of config.toml" 1; fi
if [ -z "${OUTPUT_DIR}" ]; then error_exit "fingerprints_dir not found in [paths] section of config.toml" 1; fi
...
setup_python_env "${PROJECT_ROOT}" "${VENV_PATH}" "${PYTHON_MODULE}" "${ARROW_MODULE}" "${NO_VENV}" "pandas,rich,pydantic,PIL,numpy"
...
if [ "${RESUME}" = true ]; then
    ...
    while IFS= read -r PARQUET_FILE; do
        ...
        OUTPUT_FILE="${OUTPUT_DIR}/${PARQUET_BASENAME}_fingerprints.parquet"
...
SBATCH_SCRIPT="${SCRIPT_DIR}/compute_fingerprints_array.sbatch"

# Build export string
EXPORT_VARS="ALL,PARQUET_LIST_FILE=${PARQUET_LIST_FILE},PYTHON_MODULE=${PYTHON_MODULE},LOG_DIR=${LOG_DIR},PROJECT_ROOT=${PROJECT_ROOT}"
...
SUBMIT_OUTPUT=$(sbatch \
    ...
    --array="${ARRAY_SPEC}" \
    --output="${LOG_DIR}/compute_fingerprints_%A_%a.out" \
    --error="${LOG_DIR}/compute_fingerprints_%A_%a.err" \
    --export="${EXPORT_VARS}" \
    "${SBATCH_SCRIPT}" 2>&1)
...
if echo "${SUBMIT_OUTPUT}" | grep -q "Submitted batch job"; then
    ...
    echo ""
    echo "View logs:"
    echo "  tail -f ${LOG_DIR}/compute_fingerprints_${JOB_ID}_*.out"
    echo "  tail -f ${LOG_DIR}/compute_fingerprints_${JOB_ID}_*.err"
    echo ""
