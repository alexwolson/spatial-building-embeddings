...
#
# Usage:
#   ./submit_difficulty_metadata.sh --account <ACCOUNT>
#
# Options:
...
INPUT_DIR=$(read_toml_value "${CONFIG_FILE}" "paths" "merged_dir")
OUTPUT_FILE=$(read_toml_value "${CONFIG_FILE}" "paths" "difficulty_metadata_path")

if [ -z "${INPUT_DIR}" ]; then
    error_exit "merged_dir not found in [paths] section of config.toml" 1
fi
